export default async function handler(request, response) {
  ...
  if (!body?.role?.trim() || !body?.interest?.trim() || body?.consentBetaUpdates !== true) return response.status(400).json({ error: 'Role, interest, and beta update consent are required.' });
