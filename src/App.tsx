...
import { BarChart3, BookOpen, Camera, CameraOff, Check, CircleGauge, HeartHandshake, Image, LayoutDashboard, LifeBuoy, MessageSquare, Mic, ShieldCheck, Sparkles, UserPlus, Users, Video } from 'lucide-react';
...
import { clearBehaviorProfile, loadBehaviorProfile, loadProfile, saveBehaviorProfile, saveProfile } from './utils/storage';
...
type BetaPage = 'live' | 'avatar' | 'training' | 'output' | 'privacy' | 'trust' | 'diagnostics' | 'help' | 'hub' | 'evaluation' | 'dashboard' | 'founder' | 'admin' | 'signup';
...
export function App() {
  ...
  const [page, setPage] = useState<BetaPage>('live');
  ...
  function deleteAll() { eventTracker.event('Data Delete', { scope: 'all' }); deleteAvatar(); deleteBehavior(); void deleteEidosSecureKey(); setConsent(consentManager.reset()); }
  function exportData() {
